...
const SCHOOL_TYPES = ["dress_down_day","early_dismissal","recital","field_trip","special_day","school_holiday","activity","sports_game","other"];
...
export default function TodayScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setTodayEvents(Array.isArray(evData) ? evData : []);
  ...
  async function handleDelete(eventId: number) {
    try {
      await fetch(`${API_BASE_URL}/events/${eventId}?user_id=${USER_ID}`, { method: "DELETE" });
      loadData();
    } catch (e) { console.error(e); }
  }
      ...
      try {
        if (item.type === "event") {
          await fetch(`${API_BASE_URL}/events`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_id: USER_ID, child_name: item.child_name || "all",
          ...
          await fetch(`${API_BASE_URL}/tasks`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_id: USER_ID, task_type: item.task_type || "bill",
              title: item.title, due_date: item.due_date || null, amount: item.amount || null,
              payment_url: item.payment_url || null, contact_name: item.contact_name || null }),
          });
        }
        added++;
      } catch (e) { console.error(e); }
  ...
  async function handleChat() {
    ...
    try {
      ...
      setChatInput("");
      loadData();
      if ((data.response || "").includes("✅")) setTimeout(() => setChatReply(""), 4000);
  ...
  const EventCard = ({ ev }: { ev: any }) => (
    <Swipeable renderRightActions={() => (
      <TouchableOpacity style={styles.deleteAction} onPress={() => handleDelete(ev.id)}>
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <KeyboardAvoidingView style={{ flex: 1 }}
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}
          ...
          {showChecklist && (
            <View style={styles.checklistCard}>
              ...
              <View style={styles.checkItem}>
                <View style={[styles.checkCircle, styles.checkDone]}><Text style={styles.checkMark}>✓</Text></View>
                <Text style={styles.checkText}>Sign in with Google</Text>
              </View>
              <TouchableOpacity style={styles.checkItem} onPress={() => !checklistDone.child && router.push("/(tabs)/profile")}>
              ...
              <TouchableOpacity style={[styles.checkItem, { borderBottomWidth: 0 }]} onPress={() => !checklistDone.scan && router.push("/(tabs)/profile")}>
              ...
              <View style={styles.progressBar}>
                <View style={[styles.progressFill, { width: (([checklistDone.child, checklistDone.scan].filter(Boolean).length + 1) / 3 * 100) + "%" }]} />
              </View>
            </View>
          )}
          ...
          {isRecording && <View style={styles.statusCard}><Text style={styles.statusText}>🎙️ Recording... tap mic to stop</Text></View>}
          {isTranscribing && <View style={styles.statusCard}><ActivityIndicator size="small" color="#E8734A" /><Text style={styles.statusText}>  Transcribing...</Text></View>}
          {analyzing && <View style={styles.statusCard}><ActivityIndicator size="small" color="#E8734A" /><Text style={styles.statusText}>  Analyzing with AI...</Text></View>}
          {chatReply ? (
            <View style={styles.replyCard}>
              <Text style={styles.replyText}>{chatReply}</Text>
              <TouchableOpacity onPress={() => setChatReply("")}><Text style={styles.dismiss}>Dismiss</Text></TouchableOpacity>
            </View>
          ) : null}

          <View style={styles.inputBar}>
            <TouchableOpacity style={styles.plusBtn} onPress={() => setShowPlusSheet(true)}>
            ...
            <TextInput
              ...
              multiline
            />
            <TouchableOpacity style={styles.micBtn} onPress={handleMicPress} disabled={isTranscribing}>
              {isTranscribing
                ? <ActivityIndicator size="small" color="#E8734A" />
                : <Ionicons name={isRecording ? "stop-circle" : "mic"} size={20} color={isRecording ? "#E8734A" : "#666"} />}
            </TouchableOpacity>
            {chatInput.trim() ? (
          ...
          <Text style={styles.voiceHint}>Tap 🎙️ to speak a reminder or type and tap → to send</Text>
          ...
          <View style={styles.walkthroughOverlay}>
            <View style={styles.walkthroughCard}>
              ...
              {walkthroughStep === 1 && <>
                ...
                <View style={styles.wtBtnRow}>
                  <TouchableOpacity style={styles.wtGhost} onPress={() => setWalkthroughStep(2)}>
                    <Text style={styles.wtGhostText}>Skip</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.wtPrimary} onPress={() => setWalkthroughStep(2)}>
              ...
              {walkthroughStep === 2 && <>
                ...
                <View style={styles.wtBtnRow}>
                  <TouchableOpacity style={styles.wtGhost} onPress={() => setWalkthroughStep(3)}>
                    <Text style={styles.wtGhostText}>Skip</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.wtPrimary} onPress={() => setWalkthroughStep(3)}>
                    <Text style={styles.wtPrimaryText}>Next →</Text>
                  </TouchableOpacity>
                </View>
              </>}
        ...
        <Modal visible={showPlusSheet} transparent animationType="slide">
          <TouchableOpacity style={styles.sheetBackdrop} onPress={() => setShowPlusSheet(false)} activeOpacity={1}>
            <View style={styles.sheet}>
              ...
              <View style={styles.sheetGrid}>
                <TouchableOpacity style={styles.sheetItem} onPress={handleCamera}>
                  <View style={styles.sheetIconBox}><Ionicons name="camera" size={28} color="#5C4033" /></View>
                  <Text style={styles.sheetItemLabel}>Camera</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.sheetItem} onPress={handlePhotos}>
                  <View style={styles.sheetIconBox}><Ionicons name="image" size={28} color="#5C4033" /></View>
                  <Text style={styles.sheetItemLabel}>Photos</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.sheetItem} onPress={handleFiles}>
                  <View style={styles.sheetIconBox}><Ionicons name="document-attach" size={28} color="#5C4033" /></View>
              ...
              <ScrollView style={{ maxHeight: 360 }}>
                {previewItems.map((item, i) => (
                  <TouchableOpacity key={i}
                    ...
                    onPress={() => { const u = [...previewItems]; u[i] = { ...u[i], selected: !u[i].selected }; setPreviewItems(u); }}>
                    <Ionicons name={item.selected ? "checkmark-circle" : "ellipse-outline"} size={22} color={item.selected ? "#4A9E6B" : "#C0A090"} />
                    <View style={{ flex: 1, marginLeft: 10 }}>
                      <Text style={styles.previewItemTitle}>{item.title}</Text>
                      {item.event_date ? <Text style={styles.previewItemSub}>{item.event_date}{item.event_time ? " at " + item.event_time : ""}</Text> : null}
                      {item.amount ? <Text style={styles.previewItemSub}>{item.amount}{item.due_date ? " — due " + item.due_date : ""}</Text> : null}
                      <Text style={styles.previewItemType}>{item.type === "event" ? "📅 Calendar event" : "📋 Action item"}</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </ScrollView>
              <View style={{ flexDirection: "row", gap: 10, marginTop: 16 }}>
                <TouchableOpacity style={[styles.confirmBtn, { flex: 1 }]} onPress={() => confirmItems(previewItems)}>
                  <Text style={styles.confirmBtnText}>Add {previewItems.filter(i => i.selected).length} item(s)</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.cancelBtn, { flex: 1 }]} onPress={() => setShowPreview(false)}>
...
const styles = StyleSheet.create({
  ...
  section:          { fontSize: 11, fontWeight: "700", color: "#A0856B", letterSpacing: 1.5, marginTop: 20, marginBottom: 10 },
  emptyCard:        { backgroundColor: "#F0FFF4", borderRadius: 12, padding: 12, alignItems: "center", marginBottom: 4 },
  emptyText:        { color: "#4A9E6B", fontWeight: "600", fontSize: 13 },
  eventCard:        { backgroundColor: "#fff", borderRadius: 14, padding: 14, marginBottom: 8, elevation: 2 },
  ...
  statusCard:       { flexDirection: "row", backgroundColor: "#FFF0E8", borderRadius: 12, padding: 12, marginBottom: 8, alignItems: "center", borderWidth: 1.5, borderColor: "#E8734A" },
  ...
  inputBar:         { flexDirection: "row", alignItems: "flex-end", backgroundColor: "#fff", borderRadius: 24, borderWidth: 1, borderColor: "#E8E8E8", paddingHorizontal: 8, paddingVertical: 6, marginTop: 8, elevation: 2 },
  plusBtn:          { padding: 8, marginBottom: 2 },
  inputField:       { flex: 1, fontSize: 15, color: "#333", paddingHorizontal: 4, paddingVertical: 8, maxHeight: 100 },
  micBtn:           { padding: 8, marginBottom: 2 },
  sendBtn:          { backgroundColor: "#E8734A", borderRadius: 20, width: 34, height: 34, alignItems: "center", justifyContent: "center", marginBottom: 2 },
  voiceHint:        { fontSize: 11, color: "#C0A090", marginTop: 6, textAlign: "center", fontStyle: "italic", marginBottom: 8 },
  checklistCard:    { backgroundColor: "#fff", borderRadius: 16, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: "#F5E6D3", elevation: 2 },
  ...
  checkItem:        { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 8, borderBottomWidth: 0.5, borderBottomColor: "#F5E6D3" },
  checkCircle:      { width: 20, height: 20, borderRadius: 10, borderWidth: 1.5, borderColor: "#E8734A", alignItems: "center", justifyContent: "center" },
  ...
  conflictCard:     { flexDirection: "row", alignItems: "flex-start", backgroundColor: "#FFF3E0", borderRadius: 12, padding: 12, marginBottom: 10, borderWidth: 1.5, borderColor: "#E8734A" },
  ...
  wtPrimary:        { flex: 1, backgroundColor: "#E8734A", borderRadius: 14, paddingVertical: 12, alignItems: "center" },
  wtPrimaryText:    { color: "#ffffff", fontWeight: "700", fontSize: 15 },
  wtGhost:          { flex: 1, borderWidth: 1, borderColor: "#D0D0D0", borderRadius: 14, paddingVertical: 12, alignItems: "center" },
  wtGhostText:      { color: "#999", fontSize: 15 },
  sheetBackdrop:    { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "flex-end" },
  sheet:            { backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 60, minHeight: 240 },
  ...
  sheetIconBox:     { width: 60, height: 60, borderRadius: 16, backgroundColor: "#F5F5F5", alignItems: "center", justifyContent: "center" },
  ...
  previewBox:       { backgroundColor: "#FFF8F0", borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, maxHeight: "85%" },
  previewTitle:     { fontSize: 20, fontWeight: "800", color: "#8B4513" },
  previewSubtitle:  { fontSize: 13, color: "#A0856B", marginTop: 4, marginBottom: 16 },
  previewItem:      { flexDirection: "row", alignItems: "flex-start", padding: 12, borderRadius: 12, marginBottom: 8, backgroundColor: "#fff", borderWidth: 1.5, borderColor: "#F5E6D3" },
  ...
  confirmBtn:       { backgroundColor: "#E8734A", borderRadius: 12, paddingVertical: 14, alignItems: "center" },
  confirmBtnText:   { color: "#fff", fontWeight: "700", fontSize: 15 },
  cancelBtn:        { borderWidth: 1.5, borderColor: "#C0A090", borderRadius: 12, paddingVertical: 14, alignItems: "center" },
